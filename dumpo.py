...
class MAIN:
    ...
    def __init__(self):
        ...
        try:
            self.BANNER()
            printf(Panel(f"[bold white]Please fill in the name of the group you want to search for, you can use[bold red] commas[bold white] to search for\nmultiple groups, for example:[bold green] Termux, Kerala", width=59, style="bold orange4", title="[bold orange4][ Question ]", subtitle="[bold orange4]╭───────", subtitle_align="left"))
            self.QUERY = Console().input("[bold orange4]   ╰─> ")
            if len(self.QUERY) != 0:
                printf(Panel(f"[bold white]Searching for WhatsApp groups, you can use[bold green] CTRL + C[bold white] if you want to stop searching,\nand don't use[bold red] CTRL + Z[bold white] to save the group!", width=59, style="bold orange4", title="[bold orange4][ Notes ]"))
                for NAME in self.QUERY.split(','):
                    try:
                        printf(f"[bold orange4]   ──>[bold white] LOOK FOR[bold green] {str(NAME)[:30].upper()}[bold white] GROUP!   ", end='\r')
                        time.sleep(3.0)
                        while (bool(STOP) != True):
                            try:
                                self.SEARCH_GROUPS(query=NAME)
                            except (RequestException):
                                printf(f"[bold orange4]   ──>[bold yellow] A NETWORK ERROR HAS OCCURRED!          ", end='\r')
                                time.sleep(5.0)
                                continue
                        ...
                        printf(f"[bold orange4]   ──>[bold yellow] CANCEL SEARCHING GROUP!          ", end='\r')
                        time.sleep(2.0)
                        break
                if len(GROUPS) != 0:
                    ...
                    W.close()
                    printf(Panel(f"[bold white]Congratulations, you have successfully searched for[bold red] {len(GROUPS)}[bold white] groups, and all the results have been\nsaved in[bold green] {self.FILE_NAME}[bold white]!", width=59, style="bold orange4", title="[bold orange4][ Finished ]"))
                    sys.exit()
                else:
                    printf(Panel(f"[bold red]We didn't find any groups with the name you are looking for, please try a different question!", width=59, style="bold orange4", title="[bold orange4][ Not found ]"))
                    sys.exit()
            else:
                printf(Panel(f"[bold red]You must fill in the group name correctly, it cannot be empty!", width=59, style="bold orange4", title="[bold orange4][ Cannot be Empty ]"))
                sys.exit()
        except (Exception) as e:
    ...
    def SEARCH_GROUPS(self, query):
        ...
        with requests.Session() as session:
            ...
            if response.status_code != 200 or len(response.text) == 0:
                printf(f"[bold orange4]   ──>[bold red] CURSOR NOT FOUND!             ", end='\r')
                time.sleep(3.0)
                STOP = True
                return ("-_-")
            else:
...
[bold white] Discription :[bold green] {self.DESCRIPTION}""", width=59, style="bold orange4", title="[bold orange4][ kitty mowne]"))
                        ...
                        printf(f"[bold orange4]   ──>[bold white] Gather[bold green] {str(self.GROUP_ID)[:15]}[bold white]/[bold red]{len(GROUPS)}[bold white]/[bold green]{LOOPING}[bold white] Group!   ", end='\r')
                        time.sleep(0.007)
                    else:
                        continue
...
[bold white]  \__/\  / |___|  (____  /__| /____  >______/  |   __/ 
       \/       \/     \/          \/          |__|    
       [underline green]Whatsapp Group Scanner - Coded by Dumpoothiri""", width=59, style="bold orange4"))
        return ("0_0")
